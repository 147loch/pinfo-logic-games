...
export default class SudokuSolverMethods {
  /**
   * Create the SudokuSolver with reference to
   * the SudokuGame itself as parent class.
   *
   * @param {import('./Sudoku').SudokuGame} self
   */
  constructor(self) {
    this.self = self;
  }
  ...
  findPositionWithFewestPossibilities() {
    ...
    for (let i = 0; i < SudokuConstants.BOARD_SIZE; i += 1) {
      const pos = this.self.randomBoardArray[i];
      if (this.self.solution[pos] === 0) {
        let count = 0;
        for (
          let valIndex = 0;
          ...
          if (this.self.possibilities[valPos] === 0) count += 1;
        }
        if (count < minPoss) {
          minPoss = count;
          bestPos = pos;
    ...
    for (let pos = 0; pos < SudokuConstants.BOARD_SIZE; pos += 1) {
      if (this.self.solution[pos] === 0) {
        ...
        for (
          ...
          const valPos = SudokuUtils.getPossibilityIndex(valIndex, pos);
          if (this.self.possibilities[valPos] === 0) {
            count += 1;
            lastValue = valIndex + 1;
          }
        }
        if (count === 1) {
          this.self.mark(pos, round, lastValue);
    ...
    for (let row = 0; row < SudokuConstants.ROW_COL_SEC_SIZE; row += 1) {
      for (
        ...
        if (count === 1) {
          ...
          this.self.mark(lastPos, round, value);
          return true;
        }
      }
    }
  ...
  guess(round, guessNumber) {
    ...
    for (let i = 0; i < SudokuConstants.ROW_COL_SEC_SIZE; i += 1) {
      const valIndex = this.self.randomPossibilityArray[i];
      const valPos = SudokuUtils.getPossibilityIndex(valIndex, pos);
      if (this.self.possibilities[valPos] === 0) {
        if (localGuessCount === guessNumber) {
          ...
          this.self.mark(pos, round, value);
          return true;
        }
        localGuessCount += 1;
      }
      ...
      for (
        ...
        if (inOneRow && boxRow !== -1) {
          ...
          for (let i = 0; i < SudokuConstants.ROW_COL_SEC_SIZE; i += 1) {
            ...
            const valPos = SudokuUtils.getPossibilityIndex(valIndex, pos);
            if (section !== section2 && this.self.possibilities[valPos] === 0) {
              this.self.possibilities[valPos] = round;
              doneSomething = true;
            }
          }
          if (doneSomething) {
        ...
        if (inOneBox && rowBox !== -1) {
          ...
          for (let i = 0; i < SudokuConstants.GRID_SIZE; i += 1) {
            for (let j = 0; j < SudokuConstants.GRID_SIZE; j += 1) {
              ...
              const valPos = SudokuUtils.getPossibilityIndex(valIndex, pos);
              if (row !== row2 && this.self.possibilities[valPos] === 0) {
                this.self.possibilities[valPos] = round;
                doneSomething = true;
              }
            }
        ...
        if (inOneBox && colBox !== -1) {
          ...
          for (let i = 0; i < SudokuConstants.GRID_SIZE; i += 1) {
            for (let j = 0; j < SudokuConstants.GRID_SIZE; j += 1) {
              ...
              const pos = SudokuUtils.rowColumnToCell(row2, col2);
              const valPos = SudokuUtils.getPossibilityIndex(valIndex, pos);
              if (col !== col2 && this.self.possibilities[valPos] === 0) {
                this.self.possibilities[valPos] = round;
                doneSomething = true;
              }
            }
            ...
            if (valCount2 === 2 && c1 === c3 && c2 === c4) {
              ...
              for (
                ...
                if (valIndex3 !== valIndex && valIndex3 !== valIndex2) {
                  ...
                  if (this.self.possibilities[valPos1] === 0) {
                    this.self.possibilities[valPos1] = round;
                    doneSomething = true;
                  }
                  if (this.self.possibilities[valPos2] === 0) {
                    this.self.possibilities[valPos2] = round;
                    doneSomething = true;
                  }
                }
              }
  ...
  handleNakedPairs(round) {
    for (let pos = 0; pos < SudokuConstants.BOARD_SIZE; pos += 1) {
      const possibilities = this.self.countPossibilities(pos);
      if (possibilities === 2) {
        ...
        for (let pos2 = pos; pos2 < SudokuConstants.BOARD_SIZE; pos2 += 1) {
          if (pos !== pos2) {
            const possibilities2 = this.self.countPossibilities(pos2);
            if (
              possibilities2 === 2 &&
              this.self.arePossibilitiesSame(pos, pos2)
            ) {
              if (row === SudokuUtils.cellToRow(pos2)) {
                ...
                for (
                  ...
                  if (
                    pos3 !== pos &&
                    pos3 !== pos2 &&
                    this.self.removePossibilitiesInOneFromTwo(pos, pos3, round)
                  ) {
                    doneSomething = true;
                  }
                }
                ...
                for (let i = 0; i < SudokuConstants.GRID_SIZE; i += 1) {
                  for (let j = 0; j < SudokuConstants.GRID_SIZE; j += 1) {
                    ...
                    if (
                      pos3 !== pos &&
                      pos3 !== pos2 &&
                      this.self.removePossibilitiesInOneFromTwo(
                        pos,
                        pos3,
                        round
                      )
